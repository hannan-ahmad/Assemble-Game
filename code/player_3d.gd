...
func _on_inventory_button_pressed(button: Button) -> void:
	var valid_counter_types: bool = (
		_current_counter != null and
		(_current_counter.counter_type == Counter.CounterTypes.SIMPLE or
		_current_counter.counter_type == Counter.CounterTypes.STOVE)
	)

	if valid_counter_types:
		if button.text != "":
			for i: String in indexes_dict.keys():
				if indexes_dict[i] == button.get_index():
					counter_function[i].place_item_on_counter(_current_counter)
...
func _interact_with_counter() -> void:
	match _current_counter.counter_type:
		Counter.CounterTypes.CRATE, Counter.CounterTypes.SIMPLE:

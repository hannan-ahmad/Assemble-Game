...
func place_item_on_counter(simple_counter: Counter) -> void:
	if simple_counter.current_item == null and player.current_item[_item_name] > 0:
		if player._current_counter.counter_type != CounterTypes.STOVE:
			_place_item(simple_counter)
			simple_counter.item_name = _item_name
		else:
			if _item_scene.instantiate().is_in_group("patty"):
				_place_item(simple_counter)


func _place_item(simple_counter: Counter) -> void:
